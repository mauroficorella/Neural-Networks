...
class Discriminator(tf.keras.models.Model):
    ...
    @tf.function
    def call(self, inputs, training=None, mask=None):
        x = self.D_input_block(inputs)
        x = self.downsample_128(x)
        x = self.downsample_64(x)
        x = self.downsample_32(x)
        x_16 = self.downsample_16(x)
        x_8 = self.downsample_8(x_16)
        center_cropped_x_16 = cropCenterImages(x_16, 8)
        x_img_decoded_128_center_part = self.decoder_img_part(center_cropped_x_16)
        x_img_decoded_128 = self.decoder_img(x_8)
        x_real_fake_logits = self.real_fake_out(x_8)
